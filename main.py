...
@torch.inference_mode()
def run_hypothesis(circuit, toks, correspondence, good_induction_candidates, samples=100, print=False):
    ds = Dataset({"toks_int_var": toks})
    eval_settings = ExperimentEvalSettings(device_dtype=DEVICE, run_on_all=False)
    exp = Experiment(circuit, ds, correspondence, samples, random_seed=12)
...
def main():
    ...
    res, scrubbed_circuit, mean_losses = run_hypothesis(
        with_a1_ind_inputs, toks_int_values, corr, good_induction_candidate
    )
